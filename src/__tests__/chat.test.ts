...
describe("single goblin chat", () => {
  ...
  it("exposes rites and settings in the left sidebar without chat chrome", () => {
    ...
    assert.match(serverSource, /id="sidebar-settings-card"[\s\S]*Never trust a clean cache/);
  ...
  it("loads rites from the sidebar into the main surface", () => {
    ...
    assert.match(serverSource, /res\.send\(tankHtml\(warren\.manifest\.name, loot\.length, rites\.length, drift, runs, autopilot\)\)/);
