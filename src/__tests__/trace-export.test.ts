...
describe("exportRunAsMasTrace", () => {
  ...
  it("maps scout:start/done to spawn + return for scout", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        step({ kind: "scout:start", globs: ["src/**/*.ts"] }, 0),
        step({ kind: "scout:done", morselId: "morsel-r", fileCount: 7 }, 1),
      ],
    }));
    ...
    assert.equal(ret?.agent, "scout");
    assert.equal(ret?.content_ref, "morsel-r");
  });

  it("maps swarm:start to one orchestrator_decision + N spawns", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        step({ kind: "swarm:start", size: 3 }, 0),
      ],
    }));
    const decisions = t.events.filter((e) => e.type === "orchestrator_decision");
    const spawns = t.events.filter((e) => e.type === "spawn" && typeof e.agent === "string" && e.agent.startsWith("forager#"));
    assert.equal(decisions.length, 2, "initial + dispatch_swarm");
    assert.equal(spawns.length, 3);
    assert.deepEqual(spawns.map((s) => s.agent), ["forager#0", "forager#1", "forager#2"]);
  });

  it("links swarm:forager returns to their spawn events via causal edge", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        step({ kind: "swarm:start", size: 2 }, 0),
        step({ kind: "swarm:forager", morselId: "g0", index: 0, personality: "nerdy" }, 1),
        step({ kind: "swarm:forager", morselId: "g1", index: 1, personality: "feral" }, 2),
      ],
    }));
  ...
  it("maps sting:done to a message edge from wasp to forager", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        step({ kind: "swarm:start", size: 1 }, 0),
        step({ kind: "swarm:forager", morselId: "g0", index: 0 }, 1),
        step({ kind: "sting:start" }, 2),
        step({ kind: "sting:done", foragerId: "g0", waspId: "gr0" }, 3),
      ],
    }));
    const msg = t.events.find((e) => e.type === "message" && e.agent === "wasp");
    assert.ok(msg);
    assert.equal(msg!.to, "forager#0");
  ...
  it("maps review:verdict to aggregate events with passed/score", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        ...
        step({ kind: "review:verdict", verdict: { morselId: "g0", passed: true, score: 0.8, critique: "ok" } }, 1),
      ],
    }));
  ...
  it("classifies topology as 'mixed' when specialists were spawned", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        ...
        step({ kind: "specialist:done", morselId: "s0", index: 0 }, 1),
      ],
    }));
  ...
  it("maps budget:exceeded and scribe:error to safety_event", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        step({ kind: "budget:exceeded", used: 100, cap: 50, phase: "scout" }, 0),
  ...
  it("ignores 'thinking' deltas (they are below trace abstraction)", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        step({ kind: "thinking", slot: "soldier", text: "partial..." }, 0),
        step({ kind: "thinking", slot: "soldier", text: "partial..." }, 1),
      ],
    }));
  ...
  it("unwraps plan-mode { nodeId, step } envelopes for flight steps", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        { seq: 0, kind: "plan:built", data: { plan: { nodes: [{ id: "n1" }] } } },
        { seq: 1, kind: "plan:node:start", data: { nodeId: "n1" } },
        { seq: 2, kind: "step", data: { nodeId: "n1", step: { kind: "scout:start", globs: ["src/**"] } } },
        { seq: 3, kind: "step", data: { nodeId: "n1", step: { kind: "scout:done", morselId: "L", fileCount: 3 } } },
        { seq: 4, kind: "plan:node:done", data: { nodeId: "n1", outcome: "winner", flightId: "R", artifactId: "A" } },
      ],
    }));
    // Expect scout spawn + return events to be present even though they're plan-wrapped.
    assert.ok(t.events.some((e) => e.type === "spawn" && e.agent === "scout"));
    assert.ok(t.events.some((e) => e.type === "return" && e.agent === "scout"));
  });
  ...
  it("every non-first event has an incoming temporal edge", () => {
    const t = exportRunAsMasTrace(baseRun({
      events: [
        step({ kind: "scout:start", globs: [] }, 0),
        step({ kind: "scout:done", morselId: "x", fileCount: 1 }, 1),
        step({ kind: "flight:done", outcome: "winner" }, 2),
      ],
    }));
