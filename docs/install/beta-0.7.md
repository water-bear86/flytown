...
> **Note (FLYTOWN baseline):** this page describes upstream Goblintown's own
> signed release pipeline and GitHub Release downloads. This FLYTOWN baseline
> fork has not built or published any installers of its own — there is no
> `.github/workflows/desktop-release.yml` in this repo yet, and
> `npm run release:ready` will fail here until a release pipeline exists. Use
> `npm run dist:mac` / `dist:win` / `dist:linux` (see the main
> [README](../../README.md#building-from-source)) to build a local desktop
> package from source instead. The rest of this page is kept as reference for
> how upstream Goblintown's release process works.
